...
\author{Simen Tennøe}
\institute{University of Oslo, CINPLA\\ Simula}
...
\pause
    \begin{tikzpicture}[remember picture, overlay, font=\sffamily]
      ...
      \node[align=left] at (image2.east) {\hspace{7cm} \bf Polynomial Chaos and Chaospy};
    \end{tikzpicture}

\pause

\begin{tikzpicture}[remember picture, overlay, font=\sffamily]
  \node [align=left, xshift=0\textwidth,yshift=-0.3\textwidth] (image1) at (current page.center)
...
\end{frame}
 ...
 \frametitle{Chaospy is a Python toolbox for uncertainty quantification}
     ...
     Chaospy is modular with a syntax that lies close the the mathematical theory.

\vspace{10mm}


  It implements advanced Monte Carlo methods and Polynomial Chaos expansions.
...
\begin{itemize}[<+->]
\gooditem Modular
\gooditem The syntax lies close to the mathematical theory
...
\begin{itemize}[<+->]
\baditem Modular
\baditem The syntax lies close to the mathematical theory
\baditem Slow
\end{itemize}
...
dist = cp.J(dist_a, dist_I)
|\pause|
samples = dist.sample(size=1000, rule="R")
|\pause|
